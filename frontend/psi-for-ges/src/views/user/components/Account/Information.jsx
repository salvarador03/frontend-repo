...
import { getUserMessages, getAllCountries, updateUser, deleteUser } from '../../../../services/UsuarioService';
...
const Information = () => {
    ...
    const [userData, setUserData] = useState({
        ...
        fechaNacimiento: userSession?.fechaNacimiento ? new Date(userSession.fechaNacimiento) : null,
        foto: `${apiEndpoint}/usuarios/foto/${userSession?.id}` || '',
    });
    ...
    const handleInputChange = (e) => {
        ...
        if (name === 'foto' && files.length > 0) {
            ...
            setUserData({ ...userData, foto: fileUrl });
        } else {
    ...
    const handleSave = () => {
        ...
        if (userData.fechaNacimiento && userData.fechaNacimiento > new Date()) {
            handleNotification('error', 'La fecha de nacimiento no puede estar en el futuro');
            return;
        }

        const updatedUserData = { ...userData, fechaNacimiento: userData.fechaNacimiento ? userData.fechaNacimiento.toISOString().split('T')[0] : null };

        updateUser(userSession.id, updatedUserData, photoFile)
            .then(responseData => {
                setEditMode(false);
                handleNotification('success', 'Usuario actualizado exitosamente');
                // Actualiza el token en el localStorage y vuelve a autenticar
                localStorage.setItem('authToken', responseData.token);
                login(responseData.token);
                // Actualiza el contexto del usuario con los nuevos datos
                updateUserContext(getUserFromToken(responseData.token));
                setUserData({ ...userData, foto: `${apiEndpoint}/usuarios/foto/${userSession.id}` }); // Actualiza la URL de la foto
            })
            .catch(error => {
        ...
        <div className="flex flex-col items-center min-h-screen bg-gray-100 px-4 pt-44 sm:px-6 lg:px-8">
            ...
            <div className="max-w-4xl w-full mt-10 bg-white p-8 rounded-lg shadow-lg">
                ...
                <div className="mb-4 text-center">
                    {userData.foto ? (
                        <img src={userData.foto} alt="Foto del usuario" className="w-24 h-24 rounded-full mx-auto mb-4" />
                    ) : (
                        ...
                        <div className="mb-4">
                            <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
                                ...
                                <div>
                                    ...
                                    <DatePicker
                                        selected={userData.fechaNacimiento}
                            ...
                            <div className="mb-4 text-center">
                                ...
                                {userData.fechaNacimiento && (
                                    <p className="text-zinc-700">
                                        <FaCalendarAlt className="inline-block mr-2" /> {userData.fechaNacimiento.toLocaleDateString('es-ES', { day: '2-digit', month: '2-digit', year: 'numeric' })}
                                    </p>
                                )}
                            </div>
                            <button
